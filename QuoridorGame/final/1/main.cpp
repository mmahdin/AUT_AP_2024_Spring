...
int main() {
    ...
    try {
        ...
        json::value json_data = serialize_game_state(board, turnHistory);
        ...
        while (!board->is_finished()) {
			...
			if (bestMove.wall_or_piece == 0) {
				...
				std::cout << "Player 1 Place " << bestMove.hv << " Wall at " << bestMove.newX << " " << bestMove.newY << std::endl;
			} else {
				board->movePlayer(1, bestMove.newX, bestMove.newY);
				board->use_wall(1);
                std::cout<< "Player 1 Move to " << bestMove.newX << " " << bestMove.newY << std::endl;
			}

            turnHistory.push_back(bestMove);
			json_data = serialize_game_state(board, turnHistory);
			json_str = json::serialize(json_data);
