...
template <std::size_t N = 81920>  // Default size of the Bloom filter bit array
                                  // set to 81920 bits (10 kilobytes)
class BloomFilter {
 public:
  ...
  // Overload for adding items from a file, where words are assumed to be
  // separated by ", "
  ...
  // Definitive check for an item's presence combining Bloom filter and
  // CDNServer
  ...
  BloomFilter& operator&(
      const BloomFilter& other);  // Intersection of two filters
  ...
  friend std::ostream& operator<<(
      std::ostream& os,
      const BloomFilter& bloom_filter);  // Output stream operator
  friend std::istream& operator>>(
      std::istream& is, BloomFilter& bloom_filter);  // Input stream operator
 ...
 private:
  ...
  std::size_t hash(const std::string& item, std::size_t seed) const {
    ...
    return hasher(
        item + std::to_string(
                   seed));  // Concatenate the seed to the item before hashing
  }
};
