...
Board::Board(Logger& logger) : player1(0, 4, '1'), player2(8, 4, '2'), logger(logger) {
    initializeGrid();
...
bool Board::is_possible_to_move(Player& player, int newX, int newY) {
    ...
    if (newY - player.getY() == 1 && !valid_moves[0]) {
        ...
        return false;
    } else if (player.getY() - newY == 1 && !valid_moves[1]) {
        logger.log("Move up is blocked", Logger::DEBUG);
        return false;
    } else if (newX - player.getX() == 1 && !valid_moves[2]) {
        logger.log("Move right is blocked", Logger::DEBUG);
        return false; 
    } else if (player.getX() - newX == 1 && !valid_moves[3]) {
